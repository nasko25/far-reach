export const NavBar = (props: { rank?: number, stats?: boolean}) => {
    ...
    return (
        <nav tw="text-white border-gray-200 dark:bg-gray-900 border-b border-[#e5e7eb] h-40">
          ...
          <p tw="m-4 mr-20 ml-auto my-auto p-0 max-w-lg"> { props.stats ? props.rank ? `Current Rank: ${props.rank}` : "You are not yet a part of far reach. Please apply below." : "Web3 Affiliate Platform"} </p>
        </nav>
      );
  }
  ...
  export const Stats = (props: { displayName: string | undefined, totalRewards: number | undefined, profileImage: string | undefined }) => {
    // assumes parent is display: flex and has 100% width and height
    return (
        <div tw="text-white border-gray-200 dark:bg-gray-900 mb-auto mt-5 ml-40 flex flex-col">
          ...
          <div tw="flex flex-row m-0">
            ...
            <p tw="m-0 ml-auto mr-40 w-70 text-center h-40"> Total rewards so far: ${props.totalRewards ?? 0}</p>
          </div>
          ...
          { props.totalRewards ? <FrameData/> : undefined }
          { props.totalRewards ? <FrameData/> : undefined }
          { props.totalRewards ? undefined : <p tw="mr-40 justify-center">You don't have any frames yet</p>}
        </div>
      );
  }